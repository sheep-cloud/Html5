...
2. 基本滑动
	...
	2.3. 创建BScroll对象的时机
		...
		callback + $nextTick
3. 滑动右侧列表，左侧同步更新
	3.1. better-scroll禁用了原生的dom事件，使用的是自定义事件
	3.2. 绑定监听: scroll/scrollEnd
	3.3. 滚动监听的类型: probeType
	3.4. 列表滑动的3种类型
		3.4.1. 手动触摸
		3.4.2. 惯性
		3.4.3. 编码
	...
	3.6. 编码
		3.6.1. 在滑动过程汇总，实时收集scrollY
		3.6.2. 列表第一次显示后，收集tops
		3.6.3. 实现currentIndex的计算逻辑
4. 点击左侧列表项，右侧滑动到对应位置
```
...
```ini
1. 父子组件:
	1.1. 子组件调用父组件的方法: 通过props将方法传递给子组件
	1.2. 父组件调用子组件的方法: 通过ref找到子组件标签对象
```
